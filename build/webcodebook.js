(function (global, factory) {
    ...
    function indicateLoading(codebook, event, callback) {
        ...
        var loading = setInterval(function () {
            var loadingIndicated = codebook.loadingIndicator.style('display') !== 'none';
    ...
    function layout() {
        ...
        this.loadingIndicator = this.wrap.append('div').classed('wcb-hidden', true).attr('id', 'loading-indicator');
    ...
    function init$1(codebook) {
        //indicateLoading(codebook, 'Control initialization');
    ...
    function update(codebook) {
        ...
        var filters = codebook.controls.wrap.selectAll('.filterCustom select').on('change', function (d) {
            ...
            indicateLoading(codebook, 'Filter change', function () {
    ...
    function update$1(codebook) {
        ...
        groupSelect.on('change', function () {
            ...
            indicateLoading(codebook, 'Group by change', function () {
        ...
        if (codebook.nav.tabs.length > 1) {
            ...
            navItems.on('click', function (d) {
                if (!d.active) {
                    ...
                    if (codebook.config.controlVisibility !== 'hidden') codebook.config.previousControlVisibility = codebook.config.controlVisibility;
                    if (codebook.config.controlVisibility != 'disabled') {
                        codebook.config.controlVisibility = d.controls ? codebook.config.previousControlVisibility : 'hidden';
                        codebook.controls.controlToggle.set(codebook);
                    }
                }
            });
    ...
    function highlightData(chart) {
        ...
        bars.on('click', function (d) {
            indicateLoading(codebook, 'Highlighting', function () {
    ...
    function addBoxPlot(chart) {
        ...
        if (chart.config.boxPlot) {
            ...
            for (var item in quantiles) {
                ...
                //Vertical lines
                quantile.mark = chart.svg.append('line').attr({
                    class: 'statistic',
                    x1: chart.x(quantile.quantile),
                    ...
                    stroke: [0.05, 0.95].indexOf(quantile.probability) > -1 ? 'black' : [0.25, 0.75].indexOf(quantile.probability) > -1 ? 'black' : 'black',
                    'stroke-width': '3px'
                });
            ...
            if (outliers.length < 100) {
                ...
                console.log(outliers.length + ' outliers not drawn for the following chart:');
        ...
        if (panel && !this.group) {
            ...
            if (!config.y.domain[1]) {
                ...
                nestedData.forEach(function (group) {
                    ...
                    max = Math.max(max, d3$1.max(bins, function (d) {
                        return d.values;
                    }));
                });
            }
    ...
    function makeChart(d) {
        ...
        if (d.statistics.n > 0) {
            if (d.chartType === 'horizontalBars') {
                ...
                var summary = d3$1.select(this).append('div').attr('class', 'characterSummary').html(d.summaryText);

                summary.select('span.drawLevel').on('click', function () {
                    var node = this.parentNode.parentNode.parentNode;
                    d3.select(node).select('div.characterSummary').remove();
    ...
    function renderValues(d, list) {
        ...
        if (d.type == 'categorical') {
            ...
            var valueItems = list.selectAll('li.value').data(topValues).enter().append('li').attr('class', 'value');

            valueItems.append('div').text(function (d) {
                return d.key;
            }).attr('class', 'wcb-label').attr('title', function (d) {
                return d.key;
            });
            ...
            if (d.statistics.values.length > 5) {
                ...
                var extra_span = list.append('li').attr('class', 'value').append('div').attr('class', 'wcb-label').html('and ' + extraCount + ' more.');
            }
        } else if (d.type == 'continuous') {
            var nonMissing = d.values.filter(function (f) {
                return !f.missing;
            }).map(function (m) {
                return +m.value;
            });
            var sortedValues = d3$1.set(nonMissing).values() //get unique
            .sort(function (a, b) {
                return a - b;
            }); // sort low to high
            ...
            var valueItems = list.selectAll('li.value').data(valList).enter().append('li').attr('class', 'value');
    ...
    function renderStats(d, list) {
        var ignoreStats = ['values', 'highlightValues', 'min', 'max', 'n', 'N', 'nMissing', 'percentMissing'];
    ...
    function renderMeta(d, list) {
        ...
        //render the items
        var metaItems = list.selectAll('li.meta').data(d.meta.filter(function (f) {
            return f.key != 'Type';
        })).enter().append('li').classed('meta', true).classed('hidden', function (d) {
            return d.hidden;
        });
        ...
        if (dropped.length) {
            list.append('li').attr('class', 'details').append('div').html('&#9432;').property('title', 'Meta data for ' + dropped.length + ' item(s) (' + dropped.toString() + ') were empty and are hidden.');
        }
    }

    function makeDetails(d) {
        var stat_list = d3$1.select(this).append('ul').attr('class', 'stats');
        var val_list = d3$1.select(this).append('ul').attr('class', 'values');
    ...
    function makeMeta(d) {
        var hasMeta = d.meta.filter(function (f) {
            return !f.hidden;
        }).filter(function (f) {
            return f.key.toLowerCase() != 'type';
        }).length > 0;
        if (hasMeta) {
            var meta_list = d3$1.select(this).append('ul').attr('class', 'meta');
    ...
    function makeHist(this_, d) {
        ...
        // scales
        var x = d3$1.scale.ordinal().domain(bins.map(function (d) {
            return d.key;
    ...
    function makeTitle(d) {
        ...
        */
        ...
        //add sparklines
        var sparkDiv = d3$1.select(this).append('div').attr('class', 'spark').datum(d);

        if (d.chartType != 'character') {
            sparkDiv.each(createSpark);
        }

        var type = d.type == 'continuous' ? 'continuous' : d.chartType == 'character' ? 'character' : 'categorical';
        sparkDiv.append('div').attr('class', 'sparkLabel').text(type == 'continuous' ? '#' : type == 'character' ? 'abc' : 'cat').attr('title', type + ' column');

        //add percent missing (if > 0%)
        d3$1.select(this).append('span').attr('class', 'percent-missing').text(function (d) {
            return d3$1.format('0.1%')(d.statistics.percentMissing) + ' missing';
        }).style('display', function (d) {
            return d.statistics.percentMissing == 0 ? 'none' : null;
        }).style('cursor', 'pointer').style('color', function (d) {
            return d.statistics.percentMissing >= 0.1 ? 'red' : '#999';
        }).attr('title', function (d) {
            return d.statistics.nMissing + ' of ' + d.statistics.N + ' missing. Missing values include:\n' + d.missingSummary;
        });
    }
    ...
    function renderRow(d) {
        ...
        rowWrap.append('div').attr('class', 'row-head section').append('div').attr('class', 'row-title').each(makeTitle);
        ...
        rowWrap.append('div').attr('class', 'row-meta section').each(makeMeta);
    }
    ...
    function init$6(codebook) {
        ...
        //Define table.
        dataListing.table = webcharts.createTable(codebook.wrap.select('.dataListing').node(), {});
    ...
    function draw$1(codebook) {
        //indicateLoading(codebook, 'Chart maker draw');
        ...
        if (x_obj == undefined || y_obj == undefined) {
            chartMaker.wrap.append('div').attr('class', 'status error').text('No continuous and/or group variables available to plot. Visit the settings tabs to update variable settings.');
        } else {
            ...
            //Define chart.
            chartMaker.chart = webcharts.createChart(codebook.wrap.select('.chartMaker.section .cm-chart').node(), chartMaker.chartSettings);
    ...
    function init$8(codebook) {
        ...
        if (codebook.data.summary.length > 2) {
            ...
            chartMaker.wrap.append('div').attr('class', 'status').text('Two or more variables required to use Charts module.');
        }
    }
    ...
    function setDefaults(codebook) {
            ...
            codebook.config.meta.forEach(function (m) {
                ...
                if (mKeys.indexOf('value_col') > -1 & mKeys.indexOf('type') > -1) {
                    if (['categorical', 'continuous'].indexOf(m.type.toLowerCase()) > -1) {
                        ...
                        console.log("Invalid type ('" + m.type + "') for " + m.value_col + ' specified in metadata.');
                    }
                }
            });
        ...
        codebook.config.variableTypes = codebook.config.variableTypes.filter(function (type, i) {
            var is_object = (typeof type === 'undefined' ? 'undefined' : _typeof(type)) === 'object',
                ...
                legit = legit_structure ? ['continuous', 'categorical'].indexOf(type.type) > -1 : false;
            if (!legit) console.warn('Item ' + i + ' of settings.variableType (' + JSON.stringify(type) + ') must be an object with both a "value_col" and a "type" property of "continuous" or "categorical".');

            return legit;
        });

        if (codebook.config.variableTypes.length && defaultSettings$1.variableTypes.length) {
            ...
            codebook.config.variableTypes = codebook.config.variableTypes.length ? codebook.config.variableTypes : defaultSettings$1.variableTypes;
        }
        ...
        codebook.config.maxLevels = codebook.config.maxLevels || defaultSettings$1.maxLevels;
        ...
        codebook.config.whiteSpaceAsMissing = codebook.config.whiteSpaceAsMissing == undefined ? defaultSettings$1.whiteSpaceAsMissing : codebook.config.whiteSpaceAsMissing;

        codebook.config.missingValues = codebook.config.missingValues || defaultSettings$1.missingValues;
    ...
    function makeFiltered(data, filters) {
        ...
        filters.forEach(function (filter) {
            filtered = filtered.filter(function (d) {
                var currentValues = filter.values.filter(function (f) {
                    return f.selected;
                }).map(function (m) {
                    return m.value;
                });
    ...
    function categorical(vector, sub) {
        ...
        statistics.values = d3$1.nest().key(function (d) {
            ...
            return stats;
        }).entries(nonMissing).map(function (d) {
            Object.assign(d, d.values);
            delete d.values;
            return d;
        });

        statistics.Unique = d3$1.set(nonMissing.map(function (d) {
            return d.value;
        })).values().length;
        ...
        if (sub) {
            statistics.highlightValues = d3$1.nest().key(function (d) {
                ...
                return stats;
            }).entries(nonMissing.filter(sub)).map(function (d) {
                Object.assign(d, d.values);
                delete d.values;
                return d;
            });
    ...
    function continuous(vector, sub) {
        ...
        quantiles.forEach(function (quantile) {
            ...
            statistics[statistic] = d3$1.format('0.1f')(d3$1.quantile(nonMissing, quantile[1]));
        });
    ...
    function reset(codebook) {
        indicateLoading(codebook, 'Settings reset', function () {
    ...
    function updateSettings(codebook, column) {
        var setting = column === 'Label' ? 'variableLabels' : column === 'Group' ? 'groups' : column === 'Filter' ? 'filters' : column === 'Hide' ? 'hiddenVariables' : column === 'Type' ? 'variableTypes' : console.warn('Something unsetting has occurred...');
        ...
        if (['Group', 'Filter', 'Hide'].indexOf(column) > -1) {
            ...
            var currentValues = inputs.filter(function (d) {
                d.value.value = d3$1.select(this).select(inputType).property('value');
    ...
    function layout$1(codebook) {
        ...
        var columns = codebook.data.summary.map(function (d) {
            ...
            var columnDatum = {
                ...
                Label: {
                    type: 'text',
                    value: labeledColumns.indexOf(column) > -1 ? codebook.config.variableLabels[labeledColumns.indexOf(column)].label : ''
                },
                Type: {
                    type: 'text',
                    value: typedColumns.indexOf(column) > -1 ? codebook.config.variableTypes[typedColumns.indexOf(column)].type : '',
        ...
        columnTableRows = columnTable.append('tbody').selectAll('tr').data(columnMetadata).enter().append('tr').classed('hidden', function (d) {
            ...
            switch (d.key) {
                ...
                case 'Label':
                    ...
                    cell.append('input').attr('type', d.value.type).property('value', d.value.value).on('change', function () {
                ...
                case 'Type':
                    ...
                    var typeOptions = ['automatic (' + d.value.autoType + ')', 'continuous', 'categorical'];

                    typeSelect.selectAll('option').data(typeOptions).enter().append('option').property('selected', function (opt) {
    ...
    function init$a(codebook) {
        ...
        codebook.title.countSummary = codebook.title.wrap.append('span').attr('class', 'countSummary');
    ...
    function init$b(codebook) {
        //initialize the wrapper
        codebook.title.highlight.clearButton = codebook.title.wrap.append('button').classed('clear-highlight', true).classed('hidden', codebook.data.highlighted.length == 0).text('Clear Highlighting').on('click', function () {
            codebook.data.highlighted = [];
    ...
    function updateCountSummary(codebook) {
        ...
        if (codebook.data.summary.length > 0) {
            ...
            var rowSummary = nShown + ' of ' + nTot + ' (' + percent + ') rows selected';
        } else {
            var rowSummary = 'No rows selected.';
        }

        //Add note regarding highlighted cells and show/hide the clear highlight button
        var highlightSummary = codebook.data.highlighted.length > 0 ? ' and ' + codebook.data.highlighted.length + ' <span class="highlightLegend">highlighted</span>. ' : '.';

        codebook.title.highlight.clearButton.classed('hidden', codebook.data.highlighted.length == 0);
    ...
    function init$d(codebook) {
        ...
        var showAllButton = selector.append('button').text('Show All Details').on('click', function () {
            codebook.wrap.selectAll('.variable-row').classed('hiddenDetails', false);
            codebook.wrap.selectAll('.row-toggle').html('&#9660;');
        });

        var hideAllButton = selector.append('button').text('Hide All Details').on('click', function () {
    ...
    function setDefaults$1() {
        ...
        /********************* ignoredColumns *********************/
        explorer.config.ignoredColumns = explorer.config.ignoredColumns || defaultSettings$2.ignoredColumns;
        ...
        /********************* tableConfig ***************/
        explorer.config.tableConfig = explorer.config.tableConfig || defaultSettings$2.tableConfig;
        ...
        explorer.config.tableConfig.cols = Object.keys(explorer.config.files[0]).filter(function (f) {
            ...
            return ['fileID', 'settings', 'selected', 'event', 'json'].indexOf(f) == -1;
        }); //drop system variables from table

        /********************* defaultCodebookSettings ***************/
        explorer.config.defaultCodebookSettings = explorer.config.defaultCodebookSettings || defaultSettings$2.defaultCodebookSettings;
    ...
    function onDraw$1() {
        ...
        explorer.codebook.fileListing.table.on('draw', function () {
            ...
            this.table.select('tbody').selectAll('tr').classed('selected', function (f) {
                return f.fileID === explorer.current.fileID;
            });

            //Linkify the labelColumn
            var labelCells = this.table.selectAll('tbody tr').on('click', function (d) {
                explorer.current = d;
    ...
    function addFile(label, csv_raw) {
        ...
        this.config.files = d3$1.merge([[explorer.newFileObject], this.config.files]);
    ...
    function initFileLoad() {
        ...
        explorer.dataFileLoad.wrap = explorer.codebook.fileListing.wrap.insert('div', '*').attr('class', 'dataLoader');

        explorer.dataFileLoad.wrap.append('span').text('Add a local .csv file: ');

        explorer.dataFileLoad.loader_wrap = explorer.dataFileLoad.wrap.append('label').attr('class', 'file-load-label');

        explorer.dataFileLoad.loader_label = explorer.dataFileLoad.loader_wrap.append('span').text('Choose a File');

        explorer.dataFileLoad.loader_input = explorer.dataFileLoad.loader_wrap.append('input').attr('type', 'file').attr('class', 'file-load-input').on('change', function () {
        ...
        var loadStatus = explorer.dataFileLoad.wrap.append('span').attr('class', 'loadStatus').text('');

        loadStatus.append('sup').html('&#9432;').property('title', 'Create a codebook for a local file. File is added to the data set list, and is only available for a single session and is not saved.').style('cursor', 'help');
    }
    ...
    function makeCodebook() {
        ...
        if (this.current.event == 'click') {
            this.current.settings.defaultTab = this.current.settings.tabs.map(function (tab) {
                return tab.key ? tab.key : tab;
            }).indexOf('codebook') > -1 ? 'codebook' : this.current.settings.tabs.indexOf('listing') > -1 ? 'listing' : 'files';
        }
