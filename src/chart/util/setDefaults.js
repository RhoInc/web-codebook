...
export function setDefaults(chart) {
  ...
  chart.config.autofilter = chart.config.filters.length > 0
    ? false
  ...
  chart.config.autogroups = chart.config.groups.length > 0
    ? false
  ...
  /********************* Histogram Settings *********************/
  chart.config.levelSplit =
    chart.config.levelSplit || defaultSettings.levelSplit;
}
