      ...
      codebook.config.groups
        .map(m => {
            return {
                ...
                label: codebook.data.summary
                    .filter(variable => variable.value_col === m.value_col)[0]
                    .label}; })
    ]),
    groupOptions = groupSelect.selectAll("option").data(groupLevels, d => d.value_col);
  groupOptions.enter().append("option").property('label', d => d.label).text(d => d.value_col);
