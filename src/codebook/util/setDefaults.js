...
export function setDefaults(codebook) {
    ...
    if (codebook.config.meta.length) {
        ...
        codebook.config.meta.forEach(function(m) {
            ...
            if (
                (mKeys.indexOf('value_col') > -1) &
                (mKeys.indexOf('label') > -1)
            ) {
                metaLabels.push({
        ...
        codebook.config.meta.forEach(function(m) {
            ...
            if (
                (mKeys.indexOf('value_col') > -1) &
                (mKeys.indexOf('type') > -1)
            ) {
                if (
                    ['categorical', 'continuous'].indexOf(
                        m.type.toLowerCase()
                    ) > -1
                ) {
                    metaTypes.push({
    ...
    codebook.config.filters =
        codebook.config.filters || defaultSettings.filters;
    ...
    if (
        ...
        var userLabelVars = codebook.config.variableLabels.map(
            m => m.value_col
        );
    ...
    if (codebook.config.controlVisibility !== 'disabled') {
        var startTab = availableTabs.find(
