  ...
  function indicateLoading(codebook, element, callback) {
    ...
    var loading = setInterval(function () {
      try {
        ...
        codebook.statusWrap.append('div').attr('class', 'status error').html('There was a problem updating the chart:<br>' + err);
  ...
  function makeSummary(codebook) {
    ...
    if (codebook.data.filtered.length > 0) {
      var variables = Object.keys(data[0]).map(function (variable) {
        ...
        }).indexOf(variable) > -1 ? codebook.config.variableTypes.filter(function (variableLabel) {
          return variableLabel.value_col === variable;
        ...
        var missings = varObj.values.filter(function (f) {
          return f.missing;
        }).map(function (m) {
          return m.value;
        });
        if (missings.length) {
          varObj.missingList = d3$1.nest().key(function (d) {
            return d;
          }).rollup(function (d) {
            return d.length;
          }).entries(missings).sort(function (a, b) {
            return b.values - a.values;
          });

          varObj.missingSummary = varObj.missingList.map(function (m) {
